...
const removeCategory = async (e) => {
    e.preventDefault()
    try {
        const response = await axios.delete(`http://localhost:3000/api/categories/${categoryData}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            }
          });
...
const handleChange = (e) => {
    ...
    return (
        <Container>
            ...
            <LoginForm onSubmit={addCategory}>
                <Title>Tambah Kagetori</Title>
                <FormGroup>
                    <Label name='category'>Nama Kategori:</Label>
                    <Input value={category} onChange={e => setCategory(e.target.value)} type="text" name="category" required />
                    {error && <ErrorMsg>*{response.data.fields.name.message}</ErrorMsg>}
                </FormGroup>
                <Button type="submit" >Tambahkan</Button>
            </LoginForm>
            
            <LoginForm onSubmit={removeCategory} style={{boxShadow:'0px 0px 10px red'}}>
